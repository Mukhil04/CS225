...
class KDTree
{
  ...
  private:
    ...
    void Buildtree(vector<Point<Dim>>& arr, int dim, int left, int right, KDTreeNode*& curRoot);
    /** Helper function for grading */
     ...
     */
    Point<Dim> find_median(vector<Point<Dim>>& arr, int left, int right, int index, int dim);
    int partition(vector<Point<Dim>>& arr1, int left, int right, int pivot_index, int dim);
    KDTreeNode* findNearestNeigh(Point <Dim> query, int dim, KDTreeNode* currRoot) const;
    double euclidDist(Point <Dim> p1, Point <Dim> p2) const;
